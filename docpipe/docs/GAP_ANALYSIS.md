...
remaining corpus-proof and fidelity layers still needed before a full corpus
build. The original engine-justification gaps — anchoring, structure-aware
continuity tail, and sampler hardening — were closed in the 2026-07-08 Phase
6.0 remediation; the fail-closed Tier-0 contract gate and deterministic eval
harness were also added in Phase 6.0.
...
|---|-----------|--------|-------------------------------|
| **1** | Architecture & Stage Decomposition | **PARTIAL** | Stages 0-6 all present, resumable, single-VLM (R1.1/R1.2/R1.5 met), and current-page text-layer anchoring now feeds `build_messages` with low-overlap retry. Remaining gaps are populated proof layers, not architecture. |
...
- **P1-10 — Evaluation harness + richer detectors (Dim 8, R7.4): RESOLVED for the deterministic harness 2026-07-08.** `docpipe eval` now runs offline corpus checks with pass-fraction as the headline; default reference-free cases cover Tier-0 contract, baseline repetition/compression/long-line/intra-line loops, boilerplate absence, and table rectangularity, while optional JSON specs add presence, absence, reading-order, table, contract, and baseline cases. Remaining corpus-prep work is to populate the stratified gold/citation eval set before judging the full build.
