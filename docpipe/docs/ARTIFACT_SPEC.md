...
| `last_edited` | **Recommended** | `document["last_edited"]` (`:805`, default `None`). | Recency/citation signal. **Gap to close:** `build_frontmatter` documents this key (`clean.py:8,125,132`) but does **not currently emit it** — populate from PDF mod-date / doc revision when known. |
...
1. **Frontmatter at byte 0**, `---`-delimited, all scalars quoted → satisfies C1. `title`+`version`+`last_edited` are consumed; the rest is preserved provenance. (`last_edited` shown here is the *recommended* addition; today's `build_frontmatter` omits it — see §1.2.)
...
**Key files backing this spec:** ingestion contract — `/Users/brennanconley/vibecode/wekadocs-matrix/src/ingestion/parsers/markdown_it_parser.py`, `/Users/brennanconley/vibecode/wekadocs-matrix/src/ingestion/stages/parse.py`, `/Users/brennanconley/vibecode/wekadocs-matrix/src/ingestion/extract/commands.py`; producer — `docpipe/clean.py`, `docpipe/prompts.py`, `docpipe/validate.py`, `docpipe/stitch.py`, `docpipe/output.py`, `docpipe/artifacts.py`, `docpipe/rasterize.py`, `docpipe/docpipe.toml`. Two gaps flagged for follow-up: (1) `last_edited` is documented but not emitted by `build_frontmatter`; (2) `token_overlap` is computed but left purely advisory — the spec recommends promoting it to a soft floor on text-heavy pages.
