...
class RKGServer:
    ...
    async def run_http(self, host: str = "0.0.0.0", port: int = 8080):
        """Run server with HTTP/SSE transport using native MCP SDK.

        NOTE: This uses the native mcp.server.sse module, NOT FastMCP.
        FastMCP is explicitly avoided per project requirements.
        """
        ...
        # Create SSE transport (native MCP SDK)
