Of course. I have integrated the requested changes into the implementation plan. The updated document is below.
...
4. **Create dashboards:** Grafana panels for router/reranker latencies, margins, ambiguous rate, and timeout fallbacks.
...
3. **Record baseline ambiguous rate and cross-doc mixing rate (should drop to 0% in STRICT).**
...
* **Task P2-C (Ambiguity):** Implement the API contract from the spec; add integration test asserting exact JSON schema keys. The ambiguous path must return the specified schema, and clients should handle choices deterministically.
* **Task P2-D (Scoped retrieval):** Assert that `len(set(chunk.doc_id)) == 1` in STRICT mode.
...
3. **Canary guardrail:** if `router_ambiguous_rate` increases by > +2 p.p., auto-disable strict mode via flag.
...
### 3.2 Provider Limits & Backpressure (NEW)
* Configure `max_concurrency` per instance and a bounded in-process queue.
* On overflow: return 503 with `Retry-After` or fall back to hybrid order (configurable).

### 3.3 Pipeline Changes (code touchpoints)
...
### 3.5 Threshold Calibration Utility
...
* Add case: Reranker timeout → fallback path taken; retrieval remains doc-scoped.
* Add case: Cache hit path returns identical ordering across runs (deterministic replay).

### 3.7 Cache Invalidation (NEW)
* Include `doc_epoch` and `model_version` in cache key; purge keys on re-ingest.

### 3.8 Rollout
1. Enable in canary with `sample_rate=0.2`.
2. Monitor `rerank_latency_ms`, `cache_hit_rate`.
3. **Canary guardrail:** if `router_ambiguous_rate` increases by > +2 p.p. or `reranker_timeout_rate` > 1%, auto-disable reranker via flag.
4. Gradually ramp to 1.0.
