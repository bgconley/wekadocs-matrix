Of course. I have integrated the requested changes into the expert coder guidance document. The updated version is below.
...
6. **Fail Closed on Uncertainty** — Prefer `"AMBIGUOUS"` over mixing documents. No silent widening of scope.
...
| Normalize router scores per query before comparing margins. | Compare raw router and reranker scores directly; they’re on different scales. |
...
* **Caching key**: `sha256(query + candidate_id + model_version + doc_epoch)`.
* **Timeouts**: 800 ms max per rerank call; circuit-break on 3 consecutive timeouts.
* Include explicit timeout handling and a pure-hybrid fallback:
```python
score = reranker.score(q, docs, timeout_ms=cfg.timeouts_ms)
if score.timed_out or score.error:
    log.warn("reranker_timeout", ...)
    return hybrid_order  # never expand cross-doc scope
```
...
* Router P95 ≤ 40 ms; Reranker P95 ≤ 300 ms; Combined P95 ≤ 360 ms (excludes network p99 spikes). Log P50/P95/P99.
...
* Add `feature_flags.doc_graph_tiebreak` (default false). Only consulted when `ambiguous=true`.

---

## 9. Security & Compliance
* Do **not** log query text for privacy; hash before log (`sha256(query)`).
* Store `RERANKER_API_KEY` in Kubernetes secret via vault; mount as env var — **never** commit secrets.
* Redact query text in logs when containing high-entropy tokens; honor regional data residency if using hosted reranker.
...
6. **Score Drift** — When reranker model version changes, wipe cache namespace; require re-calibration of `μᵣ`.
7. **Epoch Mismatch** — If chunks re-ingest but cache not invalidated, you’ll serve stale rerank ordering; always include `doc_epoch` in key.
