...
def search(query: str, *, verbosity="graph", filters=None):
    ...
    if cfg.reranker.enabled:
        cand_docs = jina.rerank(query, cand_docs)   # Phase 3 tie-breaker
    chosen_docs, ambiguous = choose_docs(cand_docs, router_cfg)

    if ambiguous:
        return {"status": "AMBIGUOUS", "candidates": serialize(chosen_docs)}
    ...
    if cfg.reranker.enabled:
        chunks = jina.rerank(query, chunks)[:cfg.reranker.keep_chunks]
...
def retrieve(query, top_k, filters=None, expand=True):
    f = filters or {}
    doc_tag = f.get("doc_tag")
    allowed_docs = f.get("doc_id")      # list[str]

    # Vector branch ---------------------------------------------------------
    vec_filter = []
    if doc_tag:
        vec_filter.append(FieldCondition(key="doc_tag", match=MatchValue(value=doc_tag)))
    if allowed_docs:
        vec_filter.append(FieldCondition(key="doc_id", match=MatchAny(values=allowed_docs)))
    vector_hits = qdrant.search(embed(query), top_k, Filter(must=vec_filter))

    # BM25 branch -----------------------------------------------------------
    bm25_hits = neo4j_fulltext(query, limit=top_k*3)
    if doc_tag:
        bm25_hits = [h for h in bm25_hits if h.doc_tag == doc_tag]
    if allowed_docs:
        bm25_hits = [h for h in bm25_hits if h.doc_id in allowed_docs]

    # Fusion + Expansion ----------------------------------------------------
    chunks = fuse_and_normalize(vector_hits, bm25_hits)
    if expand:
        neighbors = expand_same_doc(chunks)
        chunks   = dedup(chunks + neighbors)
