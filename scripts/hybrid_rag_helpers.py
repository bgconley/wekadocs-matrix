# -*- coding: utf-8 -*-
"""
hybrid_rag_helpers.py  (v2.2, corrected)
...
Canonical: document_id. Legacy alias: doc_id.
This module ALWAYS prefers document_id and only falls back to doc_id when needed.

Design goals:
- Backward compatible with v2.1 data, forward-ready for v2.2 schema.
- Robust to 'document_id' vs 'doc_id' mismatches (aliases supported).
- Pluggable knobs: thresholds & weights are parameters (or env-configurable).
- Community Edition friendly (no reliance on triggers/APOC).
"""
...
class Candidate:
    ...
    Note: The 'doc_id' field below is filled with the CANONICAL document_id value.
    It's named doc_id for minimal touchpoints with existing code, but it will hold
    payload.get('document_id') if present, else payload.get('doc_id').
    """
...
def expand_graph_neighbors(
    ...
    *,
    ...
    Args:
        driver: neo4j.Driver
        seed_ids: Chunk ids to expand from.
        rel_types: Relationship types to traverse.
        max_hops: Max hop distance (1..N).
        limit_per_seed: Cap neighbors per seed.
        edge_weights: Dict of type->weight; defaults provided above.
        same_doc_only: Restrict neighbors to the same doc as the seed.
        provider: Optional provider filter (requires (:Chunk {provider}) property).
        tenant: Optional tenant filter (requires (:Chunk {tenant}) property).
        undirected: If True, traverse as undirected; otherwise, outgoing only.
    """
...
# --- 4) Fuse graph scores back into Candidate list -----------------------------
...
if __name__ == "__main__":
    print("hybrid_rag_helpers.py v2.2 (corrected) provides:")
