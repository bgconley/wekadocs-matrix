# -*- coding: utf-8 -*-
"""
hybrid_rag_helpers.py  (v2.2, no providers)
...
Canonical identifier: `document_id`. A legacy alias `doc_id` still exists in source
systems, and this helper now exposes `document_id` directly while keeping a
read/write `doc_id` alias for backward compatibility.

Designed to be imported inside your retriever service or a notebook.
"""
...
class Candidate:
    ...
    Canonical identifier is `document_id`. The `doc_id` alias property remains for callers
    that still reference the legacy name.
    """

    id: str
    text: str
    document_id: Optional[str] = None  # canonical document identifier
...
def budgeted_pack(
    ...
    *,
    ...
    for c in cand_list:
        ...
        doc = c.document_id or "__NODOC__"
...
def example_retrieve_pack(
    *,
    ...
    for c in seeds:
        if c.document_id and c.document_id not in seen_doc:
            c.is_seed = True
            seen_doc.add(c.document_id)
...
if __name__ == "__main__":
    print("hybrid_rag_helpers.py v2.2 (no providers) provides:")
