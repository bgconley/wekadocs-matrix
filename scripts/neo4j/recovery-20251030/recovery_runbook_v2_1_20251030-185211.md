# Recovery runbook — Phase 7E-4 / GraphRAG v2.1
...
docker exec nutanix-neo4j cypher-shell -u $NEO4J_USER -p $NEO4J_PASS "SHOW CONSTRAINTS; SHOW INDEXES;" > /backups/neo4j_schema_before.txt
```

## 3) Apply canonical Neo4j DDL (idempotent)
Apply the generated file: `/mnt/data/neo4j_ddl_v2_1_idempotent_20251030-185211.cypher`
```bash
docker cp /mnt/data/neo4j_ddl_v2_1_idempotent_20251030-185211.cypher nutanix-neo4j:/tmp/ddl.cypher
docker exec nutanix-neo4j cypher-shell -u $NEO4J_USER -p $NEO4J_PASS -f /tmp/ddl.cypher
```
...
sleep 10 && docker logs nutanix-mcp-server --tail 200 | sed -n '/Running startup health checks/,$p'
```
