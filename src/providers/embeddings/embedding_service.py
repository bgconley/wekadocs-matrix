...
class EmbeddingServiceProvider(EmbeddingProvider):
    ...
    def __init__(
        ...
        *,
        ...
        if settings is None:
            raise ValueError(
                "Embedding settings are required for EmbeddingServiceProvider."
            )

        self._settings = settings
        self._dims = settings.dims
        self._model_id = settings.model_id
        self._provider_name = settings.provider or "embedding-service"
        ...
        if client is not None:
            ...
            if not resolved_url:
                raise RuntimeError(
                    "EMBEDDING_BASE_URL is required for the embedding service. "
                    "Set EMBEDDING_BASE_URL to the unified gateway URL "
                    "(e.g., http://10.25.0.50:8080)."
                )
            ...
            try:
                ...
                raise RuntimeError(
                    f"Failed to initialize EmbeddingClient: {exc}"
                ) from exc
